...
static PyObject *Matrix61c_add(Matrix61c* self, PyObject* args) {
    ...
    int add_failed = add_matrix(result, self->mat, other->mat);
    if (add_failed == -1) {
	PyErr_SetString(PyExc_TypeError, "Matrix dimensions must match!");
	return NULL;
    }
...
static PyObject *Matrix61c_sub(Matrix61c* self, PyObject* args) {
    ...
    int sub_failed = sub_matrix(result, self->mat, other->mat);
    if (sub_failed == -1) {
	PyErr_SetString(PyExc_TypeError, "Matrix dimensions must match!");
	return NULL;
    }
...
static PyObject *Matrix61c_multiply(Matrix61c* self, PyObject *args) {
    ...
    int mul_failed = mul_matrix(result, self->mat, other->mat);
    if (mul_failed == -1) {
	PyErr_SetString(PyExc_TypeError, "Matrix dimensions must match!");
	return NULL;
    }
...
static PyObject *Matrix61c_neg(Matrix61c* self) {
    ...
    int neg_failed = neg_matrix(result, self->mat);
    if (neg_failed == -1) {
	PyErr_SetString(PyExc_TypeError, "Matrix dimensions must match!");
	return NULL;
    }
...
static PyObject *Matrix61c_abs(Matrix61c *self) {
    ...
    int abs_failed = abs_matrix(result, self->mat);
    if (abs_failed == -1) {
	PyErr_SetString(PyExc_TypeError, "Matrix dimensions must match!");
	return NULL;
    }
...
static PyObject *Matrix61c_pow(Matrix61c *self, PyObject *pow, PyObject *optional) {
    ...
    if (!PyLong_Check(pow)) {
	PyErr_SetString(PyExc_TypeError, "Power must be integral!");
	return NULL;
    }
    int pow_failed = pow_matrix(result, self->mat, PyLong_AsLong(pow));
    if (pow_failed == -1) {
	PyErr_SetString(PyExc_TypeError, "Matrix must be square!");
	return NULL;
    }
    if (pow_failed == -2) {
	PyErr_SetString(PyExc_TypeError, "Power must be nonnegative!");
	return NULL;
    }
...
static PyObject *Matrix61c_set_value(Matrix61c *self, PyObject* args) {
    PyObject *row_obj = NULL, *col_obj = NULL, *val_obj = NULL;
    if (PyArg_UnpackTuple(args, "args", 3, 3, &row_obj, &col_obj, &val_obj)) {
        if (!PyLong_Check(row_obj) || !PyLong_Check(col_obj) || !PyFloat_Check(val_obj)) {
            PyErr_SetString(PyExc_TypeError, "Arguments must be of type (int, int, double)!");
            return NULL;
        }
	...
	set(self->mat, row, col, val);
        ...
        PyErr_SetString(PyExc_TypeError, "Invalid number of arguments: (row, col, val)!");
        return NULL;
    }
...
static PyObject *Matrix61c_get_value(Matrix61c *self, PyObject* args) {
    PyObject *row_obj = NULL, *col_obj = NULL;
    if (PyArg_UnpackTuple(args, "args", 2, 2, &row_obj, &col_obj)) {
        if (!PyLong_Check(row_obj) || !PyLong_Check(col_obj)) {
            PyErr_SetString(PyExc_TypeError, "Arguments must be of type (int, int)!");
            return NULL;
        }
        int row = PyLong_AsLong(row_obj);
	int col = PyLong_AsLong(col_obj);
	...
	return PyFloat_FromDouble(get(self->mat, row, col));
        ...
        PyErr_SetString(PyExc_TypeError, "Invalid number of arguments: (row, col)!");
        return NULL;
    }
}
...
static PyMethodDef Matrix61c_methods[] =
    {
     {"set", (PyCFunction)&Matrix61c_set_value, METH_VARARGS | METH_KEYWORDS, "(rows, cols, val)"},
     {"get", (PyCFunction)&Matrix61c_get_value, METH_VARARGS | METH_KEYWORDS, "(rows, cols)"},
     {NULL}
    };
